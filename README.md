...
The repo is bigger than just the app — this map points you at the right place:
...
OpenCode Mobile does not collect personal data. Optional telemetry — Sentry crash reporting and PostHog activation analytics — is **opt-in, off by default**, gated behind a single consent dialog shown on first launch; if you decline, no analytics SDK is initialised and nothing leaves the device. Credentials are stored exclusively on-device in the OS keystore.
